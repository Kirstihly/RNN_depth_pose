...
def image_gradient_direction(img):
    eps = 0.000000000001
    sobelx = cv.Sobel(img,cv.CV_64F,1,0,ksize=5)+eps
...
def projective_inverse_warp_v2(img, depth,depth_src, mask, pose, intrinsics,format='eular'):
  """Inverse warp a source image to the target image plane based on projection.
  ...
  output_img,wmask = bilinear_sampler(img, src_pixel_coords)
  new_mask, _ = bilinear_sampler(mask, src_pixel_coords)

  out_depth = consistent_depth_loss(depth_src, src_pixel_coords)

  return output_img, wmask, rigid_flow, new_mask, out_depth, src_depth
...
def consistent_depth_loss(src_depth, coords):
  """Construct a new image by bilinear sampling from the input image.
  ...
  src_depth = tf.expand_dims(src_depth, axis=3)
  def _repeat(x, n_repeats):
    rep = tf.transpose(
        tf.expand_dims(tf.ones(shape=tf.stack([
            n_repeats,
...
def detect_reflection(img):
    ...
    mask = tf.where(tf.greater_equal(img,0.7), tf.ones_like(img), tf.zeros_like(img))
    kernel = tf.ones([7,7,3])
